...
let rec interpret_instr (env: venv) : instr -> venv = function
  ...
  | Iwhile (c, b)   -> 
                    while match interpret_expr env c with
                             | Bool b -> b
			     | _      -> failwith "Expression mal formée"
		       do interpret_block env b done; env
  | Ifor (e1,e2,e3,b) -> interpret_expr env e1;
                        while match interpret_expr env e2 with
                             | Bool b -> b
           | _      -> failwith "Expression mal formée"
           do interpret_block env b;
              interpret_expr env e3 done; env
