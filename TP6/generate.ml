  ...
  match e with
    ...
    | Evar var ->
      ...
      match var with
	   ...
	   | Param (n, _) -> failwith " sw $s%d, %d($sp)\n" n n
	   | Local_var (n, _)  -> printf "  sw $s%d, %d($sp)\n" n n
      end
...
and generate_call : Astv.call -> unit = function
  | (Function(n,func),exprlist)->
  printf "sub   $sp, $sp, 4
  sw  $t0, 0($sp)";
  List.map generate_expr exprlist;
  printf "jal %s\n" func
  (* On a besoin ici du code correspondant aux actions de l'appelant. *)
(*let rec print_expr exprlist = 
  match exprlist with
  | e:er -> generate_expr e; print_expr er
  | []-> printf "\n"*)
...
let rec generate_instr : instr -> unit = function
  ...
  | Iassign (var, e) ->
	...
	printf "  la $a1, %s\n  sw $a0, 0($a1)\n" (get_label var)
      | Local_var (n, id) ->
    printf "  sw $s%d, %s\n" n id
      | Param _ -> printf" not fini the parm\n"
    end
...
let generate_fun (fdescr: fun_descr) : unit =
  (* On a besoin ici du code correspondant aux actions de l'appelée. *)
   begin
  let blk=fdescr.body in
  printf "sub   $sp, $sp, 4\n  sw  $t0, 0($sp)\n";
  generate_block blk;
  printf "jal %s\n" (get_fun_label fdescr.name)
  end
